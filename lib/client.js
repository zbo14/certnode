...
class Client {
  ...
  exportAccountKeyPair (dirname, passphrase) {
    const privateKeyData = util.exportPrivateKey(this.accountPrivateKey, passphrase)
    const publicKeyData = util.exportPublicKey(this.accountPublicKey)
  ...
  async generateAccountKeyPair () {
    const { privateKey, publicKey } = await generateKeyPair(JWS_ALGORITHM)
   ...
   * @param  {String} [passphrase] - optional passphrase used to encrypt private key for CSR
   *
   * @return {Promise}
   */
  async generateCertificate (domain, email, passphrase) {
    ...
    const { certificate, privateKey } = await this.finalizeOrder(finalizeUrl, domain, email, passphrase)
  ...
  async importAccountKeyPair (dirname, passphrase) {
    ...
    this.accountPrivateKey = util.importPrivateKey(privateKeyData, passphrase)
    this.accountPublicKey = util.importPublicKey(publicKeyData)
